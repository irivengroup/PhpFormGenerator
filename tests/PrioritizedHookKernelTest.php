...
final class PrioritizedHookKernelTest extends TestCase
{
    public function testHooksRunInDescendingPriorityOrder(): void
    {
        ...
        $kernel->register(new class($captured) implements \Iriven\Fluxon\Domain\Contract\FormHookInterface {
            ...
            public function __invoke(\Iriven\Fluxon\Domain\Form\Form $form, array $context = []): void { $this->captured->append('low'); }
        }, 10);
        $kernel->register(new class($captured) implements \Iriven\Fluxon\Domain\Contract\FormHookInterface {
            /** @param ArrayObject<int, string> $captured */
            public function __construct(private ArrayObject $captured) {}
            public static function getName(): string { return 'before_render'; }
            public function __invoke(\Iriven\Fluxon\Domain\Form\Form $form, array $context = []): void { $this->captured->append('high'); }
        }, 100);
